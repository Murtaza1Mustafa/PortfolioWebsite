...
export default function Projects() {
  ...
  // Store image natural dimensions for each project
  ...
  // Dynamically load each image to get its natural dimensions
  useEffect(() => {
    projects.forEach((project) => {
      if (project.thumbimage && !imageSizes[project.id]) {
  ...
  return (
    <div className="relative">
      ...
      <main className="relative w-full min-h-screen flex flex-col items-center gap-12 bg-[#f0fbff] p-6 md:p-10 z-0">
        ...
        {/* Projects grid */}
        <div className="grid gap-8 w-full max-w-4xl 4xl:max-w-6xl">
          {filteredProjects.map(project => (
            <Link key={project.id} href={`/Projects/${project.id}`} className="block">
              <div className="flex flex-col bg-[#00b4d8] rounded-2xl shadow-lg shadow-[#0077b6]/40 p-4 md:p-6 2xl:p-8 hover:scale-[1.05]  transition-all duration-10 cursor-pointer">
                ...
                {/* Tag */}
                <p className="text-base md:text-lg lg:text-xl xl:text-xl 2xl:text-2xl text-[#ade8f4] drop-shadow-lg leading-relaxed mb-4 px-2 md:px-6">
                  Type:     <span className="bg-[#0077b6] text-white px-3 py-1 max-w-xl rounded-full text-md xl:text-lg">{project.type}</span>
                </p>

                {/* Tech Stack */}
                <p className="text-base md:text-lg lg:text-xl xl:text-xl 2xl:text-2xl text-[#ade8f4] drop-shadow-lg leading-relaxed mb-4 px-2 md:px-6">
                  Tech:      
       {project.techStack.map((tech) => (<span key={tech} className="bg-white text-[#0077b6] px-3 py-1 mx-0.5 rounded-full text-sm xl:text-md">
         {tech}
        </span>
          ))}
                </p>
                ...
                {/* Thumbnail */}
                {project.thumbimage && imageSizes[project.id] && (
                  <div className="w-full max-w-3xl mx-auto overflow-hidden rounded-xl  shadow-lg mt-4">
                    <Image
                      src={project.thumbimage}
                      alt={project.title}
                      width={imageSizes[project.id].width}
                      height={imageSizes[project.id].height}
